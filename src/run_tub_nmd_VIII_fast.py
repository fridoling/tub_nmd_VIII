...
genotype_in = np.zeros(8)
ancestor = '010'
ancestor_dec = int(ancestor, 2)
genotype_in[ancestor_dec] = params['pop_size']

res = {}
for label in ['cdc20x1', 'cdc20x4']:
    res[label] = {}
    gt_array = np.zeros((params['repeats'], params['generations'], 8))
    gf_array = np.zeros((params['repeats'], params['generations'], 3))
    mf_array = np.zeros((params['repeats'], params['generations']))
    sys.stdout.write('run experiment '+label+'\n')
    for n in range(params['repeats']):
        gt_array[n,0,:] = genotype_in
        for i in progressbar(range(1, params['generations']), "Repeat "+str(n+1), 40):    
            gt_mut = mutate(gt_array[n,i-1,:], convert_mut(mut_prob[label]))
            gt_prop = propagate(gt_mut, convert_fitness(fitness[label]))
            gt_array[n,i,:] = gt_prop
    gt_array = gt_array/gt_array.sum(axis=2)[:,:,None]
    gf_array = get_gene_freqs3(gt_array)
    mf_array = get_mean_fitness3(gt_array, convert_fitness(fitness[label]))
